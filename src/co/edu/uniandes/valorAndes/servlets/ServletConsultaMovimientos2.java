...
public class ServletConsultaMovimientos2 extends HttpServlet
{
	...
	{
		System.out.println("Respondiendo ServletConsultar Movimientos sin Criterio");
		...
		try {   			
			...
			if(!valor.equals(""))
			{
				cab.consultarMovimientosSinCriterio(pw, valor, "VALOR", fechaI, fechaF);
			}
			else
			{
				if(!tipoValor.equals(""))
				{
					cab.consultarMovimientosSinCriterio(pw, tipoValor, "TIPO_VALOR", fechaI, fechaF);
				}
				else
				{
					if(!tipoRentabilidad.equals(""))
					{
						cab.consultarMovimientosSinCriterio(pw, tipoRentabilidad, "TIPO_RENTABILIDAD", fechaI, fechaF);
					}
					else
					{
						if(!inversionista.equals(""))
						{
							cab.consultarMovimientosSinCriterio(pw, inversionista, "INVERSIONISTA", fechaI, fechaF);
						}
						else
						{
							if(!oferente.equals(""))
							{
								cab.consultarMovimientosSinCriterio(pw, oferente, "OFERENTE", fechaI, fechaF);
							}
							else
							{
